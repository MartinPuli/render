#!/usr/bin/env python3
"""
render_view.py — Renderiza una vista concreta de la escena (headless, con el
binario de Blender), para poder comparar contra una foto de Google Street View
sin depender del blender-mcp vivo.

Uso:
  # Vista a nivel de calle (para comparar con streetview/heading_XXX.jpg):
  blender -b -P render_view.py -- scene.json out.png \
          --street --heading 0 --x 0 --y 0 --height 2.5 --fov 78

  # Vista aerea 3/4 (maqueta):
  blender -b -P render_view.py -- scene.json out.png

Coordenadas del scene.json: metros, X=este, Y=norte, Z=arriba.
"""
import json
import math
import sys
...
import bpy  # noqa: F401  (garantiza que corremos dentro de Blender)
...
def parse_argv():
    ...
    if len(argv) < 2:
        sys.exit("Uso: render_view.py scene.json out.png [--street --heading H --x X --y Y --height Z --fov F] [--samples N] [--res W H]")

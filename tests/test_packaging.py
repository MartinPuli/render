"""
Tests de F4: configuracion (cityconfig), logging (citylog) y cache en disco
(citycache). No requieren Blender ni Overpass. Usan tmp_path/monkeypatch de
pytest para no tocar el entorno real del usuario.
...
def test_config_missing_file_is_safe(monkeypatch, tmp_path):
    _clear_env(monkeypatch)
    cfg = cityconfig.load_config(str(tmp_path / "no-existe.json"))
    assert cfg["radius"] == 250  # sin romper: quedan los defaults
...
def test_config_json_override(monkeypatch, tmp_path):
    ...
    assert cfg["radius"] == 500     # el archivo pisa el default
...
def test_config_env_overrides_file(monkeypatch, tmp_path):
    ...
    cfg = cityconfig.load_config(str(p))
    assert cfg["radius"] == 999     # el entorno tiene la ultima palabra


# --- citylog: tipo + nivel + idempotencia ---
...
def test_logger_idempotent_no_duplicate_handlers():
    ...
    assert a is b               # mismo logger
    assert len(b.handlers) == before  # no duplica handlers


# --- citycache: round-trip bajo un dir temporal ---
...
def test_cache_put_then_get_roundtrip(monkeypatch, tmp_path):
    ...
    assert citycache.get(key) is None          # todavia no existe
    path = citycache.put(key, "RESPONSE-BODY")  # crea el dir perezosamente
